...
void startCommMgr(void const *argument) {
	...
	{
		status = HAL_UART_Receive(&huart1, header, FRAME_HEADER_SIZE, TIMEOUT);
		...
		if (msgType == MSG_TYPE_CLEF) {
			role = ROLE_TRANSIT;
			status = HAL_UART_Receive(&huart1, ioBuffer, msgSize, TIMEOUT);
			...
			status = HAL_UART_Transmit(&huart1, header, FRAME_HEADER_SIZE, TIMEOUT);

			if (status != HAL_OK)
				continue;

			status = HAL_UART_Transmit(&huart1, ioBuffer, 1, TIMEOUT);
			...
			status = HAL_UART_Transmit(&huart3, header, FRAME_HEADER_SIZE, TIMEOUT);

			if (status != HAL_OK)
				continue;

			sprintf((char *) ioBuffer, "%" PRIu8, writeKey);
			status = HAL_UART_Transmit(&huart3, ioBuffer, 1, TIMEOUT);

			if (status != HAL_OK)
				continue;

			status = HAL_UART_Receive(&huart3, ioBuffer, FRAME_HEADER_SIZE, TIMEOUT);
			...
			HAL_UART_Receive(&huart3, ioBuffer, msgSize, TIMEOUT);

		} else if (msgType == MSG_TYPE_MSG) {

			status = HAL_UART_Receive(&huart3, ioBuffer, msgSize, TIMEOUT);
			...
			if (role == ROLE_OUTPUT) {
				HAL_UART_Transmit(&huart2, ioBuffer, msgSize, TIMEOUT);
				continue;
			}
			...
			for (uint8_t fails = 0; fails < 3; fails++)
			{
				...
				status = HAL_UART_Transmit(&huart3, ioBuffer, msgSize, TIMEOUT);

				if (status != HAL_OK) {
					goto commMgr_i;
				}

				status = HAL_UART_Receive(&huart3, header, FRAME_HEADER_SIZE, TIMEOUT);
